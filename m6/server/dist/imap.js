...
class Worker {
    ...
    async connectToServer() {
        const client = new ImapClient(this.serverInfo.imap.host, this.serverInfo.imap.port, this.serverInfo.imap);
    ...
    async listMessages(inMailbox) {
        ...
        const box = await client.selectMailbox(inMailbox);
        const ids = box.messages.total;
        const list = await client.listMessages(inMailbox, "1:" + ids, ["uid", "envelope"]);
        for (const message of list) {
            messages.push({
                id: message.uid,
                from: message.envelope.from[0].address,
    ...
    async getMessageBody(inOptions) {
        const client = await this.connectToServer();
        const messages = await client.listMessages(inOptions.mailbox, inOptions.id.toString(), ["body[]"]);
        const parsed = await (0, mailparser_1.simpleParser)(messages[0]["body[]"]);
        await client.logout();
        return parsed.text || "";
    }
    async deleteMessage(inOptions) {
        const client = await this.connectToServer();
        await client.deleteMessages(inOptions.mailbox, [inOptions.id]);
        await client.logout();
    }
}
